...
rf2_pyfunc_model = mlflow.pyfunc.load_model(model_uri="runs:/"+sklearnRunID+"/random-forest-model-preprocess")
...
class RF_with_preprocess(mlflow.pyfunc.PythonModel):
    ...
    def preprocess_input(self, model_input):
        ...
        X_test_processed["trunc_lat"] = X_test_processed['latitude'].round(2)
        X_test_processed["trunc_long"] = X_test_processed['longitude'].round(2)
        X_test_processed["review_scores_sum"] = X_test_processed[['review_scores_accuracy', 'review_scores_cleanliness', 'review_scores_checkin',
...
class RF_with_postprocess(mlflow.pyfunc.PythonModel):
    ...
    def postprocess_result(self, results):
        ...
        for item in results:
            if item > 100:
                result_cat.append("Expensive")
            else:
                result_cat.append("Not Expensive")
        return result_cat
